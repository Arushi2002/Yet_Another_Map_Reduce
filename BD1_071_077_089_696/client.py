...
while(True):
    ...
    if oper==1:
        ...
        hash_file=input("Enter the name of modular hash file(output in range of 0-len(network)): ")
        ...
        if response.status_code==200:
            ...
            #Hash
            for node in network:
                myobj={"input_file":f'partition_{input_file[:-4]}_node_{node}_map.txt', "hash":hash_file, "network": network}
                url=f'http://127.0.0.1:{node}/hasher'
                x = requests.post(url, json = myobj)
                if x.status_code==201:
                    msg=x.json()
                    print(msg['message'])
